...
function Pdf(props){
  ...
  const onDownload=()=>{
    ...
    const buffer = props.ordonnance.data;
    ...
    // const url = window.URL.createObjectURL(new Blob([props.ordonnance.data]));
    // const link = document.createElement('a');
    // link.href = url;
  ...
  useEffect(()=>{
    const extractPdfInfo = async () => {
        try {
       // const pdf = await pdfjs.getDocument({ data: props.ordonnance.data }).promise;
       // const numPages = pdf.numPages;
       
        // Déterminez la taille du fichier (optionnel, car cela nécessite un accès direct au fichier)
        const size = props.ordonnance.data.length;
        // get Size in MB
        const sizeInMegabytes = (size / (1024 * 1024)).toFixed(2);
        
        setFileSize(sizeInMegabytes);
            // const loadingTask = pdfjs.getDocument({ data: props.ordonnance.data });
            // const pdfDocument = await loadingTask.promise;
            // setNumPages(pdfDocument.numPages);
            // setFileSize(props.ordonnance.data.byteLength);
        } catch (error) {
  ...
  const displayDocument=()=>{
    props.onShowPDF(props.ordonnance.data);
  }
